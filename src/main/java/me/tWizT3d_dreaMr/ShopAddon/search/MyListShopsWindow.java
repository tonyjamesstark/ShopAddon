...
public class MyListShopsWindow extends ShopGuiWindow {
    ...
    public static final int ITEMS_IN_FULL_PAGE = 36;  // 36 items is a full page in the inventory (moved to constant)
    ...
    protected void makeMenuBarUpper() {
        ...
        ShopGuiHandler.GuiIcon guiIcon =
                Shop.getPlugin().getGuiHandler().getIconFromOption(player, PlayerSettings.Option.GUI_SORT);
        ...
        guiIcon =
                Shop.getPlugin().getGuiHandler().getIconFromOption(player, PlayerSettings.Option.GUI_FILTER_SHOP_TYPE);
        ...
        guiIcon =
                Shop.getPlugin().getGuiHandler().getIconFromOption(player, PlayerSettings.Option.GUI_FILTER_SHOP_STOCK);
        ...
        if (Shop.getPlugin().allowCreativeSelection()) {
            ItemStack searchIcon =
                    Shop.getPlugin().getGuiHandler().getIcon(ShopGuiHandler.GuiIcon.HOME_SEARCH, null, null);
    ...
    protected static ArrayList<AbstractShop> applyFilters(ArrayList<AbstractShop> toFilter, List<Predicate<AbstractShop>> filters) {
        return new ArrayList<>(toFilter.stream().filter(filters.stream().reduce(Predicate::and).orElse(x -> true)).toList());
    }
    ...
    protected List<Predicate<AbstractShop>> collectFilters(){
        ...
        ArrayList<Predicate<AbstractShop>> filters = new ArrayList<Predicate<AbstractShop>>(); // refactored filtering
        ...
        switch (guiFilterTypeIcon) {
            ...
            case MENUBAR_FILTER_TYPE_SELL:
                filters.add(shop -> (shop.getType() == ShopType.SELL || shop.getType() == ShopType.COMBO));
                break;
            case MENUBAR_FILTER_TYPE_BUY:
                filters.add(shop -> (shop.getType() == ShopType.BUY || shop.getType() == ShopType.COMBO));
                break;
