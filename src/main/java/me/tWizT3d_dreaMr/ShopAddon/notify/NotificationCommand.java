...
public class NotificationCommand {
    ...
    public enum NotificationSubcommand {
        SaleOwnerNotification(
                ...
                GuiIcon.SETTINGS_NOTIFY_OWNER_OFF), // TODO: fix - better to get these from a
        // default PlayerSettings obj
        UserNotifcation(
                "User Notifications",
    ...
    public static void Command(Player p, String args[]) {
        ...
        if (main.plugin.getConfig().getBoolean("Gui.Settings")) {
            ...
            Shop.getPlugin()
                    .getGuiHandler()
                    .setWindow(p, new PlayerSettingsWindow((((OfflinePlayer) p).getUniqueId())));
            return;
        } else {
    ...
    public static TextComponent MessageBuilder(String Message, String command) {
        ...
        message.setClickEvent(
                new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/shopnotifications " + command));
        message.setHoverEvent(
                new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text("[Click to change]")));
        return message;
    }
    ...
    private static String colorFromSetting(PlayerSettings s, NotificationSubcommand c) {
        return ""
                + (s.getGuiIcon(c.option).equals(c.defaultIcon) ? ChatColor.RED : ChatColor.GREEN)
                + c.toString();
    }
}
