...
public class guiListener implements Listener {
    ...
    public void signOpenGui(PlayerInteractEvent e) {
        ...
        if (e.getClickedBlock().getBlockData() instanceof WallSign) {
            AbstractShop shop =
                    Shop.getPlugin().getShopHandler().getShop(e.getClickedBlock().getLocation());
            if (shop == null || !shop.isInitialized()) return;

            if (p.isSneaking())
                if (e.getAction() == Action.RIGHT_CLICK_BLOCK)
                    click = ShopClickType.SHIFT_RIGHT_CLICK_SIGN;
                else click = ShopClickType.SHIFT_LEFT_CLICK_SIGN;
            else if (e.getAction() == Action.RIGHT_CLICK_BLOCK)
                click = ShopClickType.RIGHT_CLICK_SIGN;
            else click = ShopClickType.LEFT_CLICK_SIGN;
            ShopAction action = Shop.getPlugin().getShopAction(click);
            if (action == null
                    || !(action == ShopAction.TRANSACT || action == ShopAction.TRANSACT_FULLSTACK))
                return;
            e.setCancelled(true);
            Guis.makeGui(p, shop);
        }
    ...
    public void InventoryClick(InventoryClickEvent e) {
        ...
        if (e.getAction() == InventoryAction.MOVE_TO_OTHER_INVENTORY)
            if (i == 26 || i == 0 || i == 18 || i == 9 || i == 0 || i == 17 || i == 8) {
                ...
                Shop.getPlugin()
                        ...
                        .executeTransactionFromEvent(event, shop, true);
                gui.update();
            }
        if (e.getAction() == InventoryAction.PICKUP_ALL)
            if (i == 26 || i == 0 || i == 18 || i == 9 || i == 0 || i == 17 || i == 8) {
                ...
                Shop.getPlugin()
                        .getTransactionHelper()
                        .executeTransactionFromEvent(event, shop, false);
                gui.update();
            }
        e.setCancelled(true);
    }
