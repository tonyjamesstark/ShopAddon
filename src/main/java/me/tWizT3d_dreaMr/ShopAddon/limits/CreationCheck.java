...
public class CreationCheck {
    ...
    public CreationCheck(YamlConfiguration from, boolean wh) {
        ...
        for (String title : section.getKeys(false)) {
            ...
            Filter f =
                    new Filter(shopType, name, lore, listType, title, material, priceMin, priceMax);
            filters.add(f);
        }
    ...
    public MatchType test(ItemStack i, double price, int amount, ShopType st) {
        for (Filter f : filters) {
            ...
            if (i.hasItemMeta()) {
                ...
                if (im.hasDisplayName()) name = ChatColor.stripColor(im.getDisplayName());
                if (im.hasLore()) lore = ChatColor.stripColor(im.getLore().toString());
            }
