package me.tWizT3d_dreaMr.ShopAddon.util;
...
public class Filter {
	...
	public Filter(String shoptype, String name, String lore, String listType, String title,
		...
		if (isInvalid)
			Bukkit.getLogger().log(Level.INFO, this.title + " is Invalid");
	}
	...
	private Double getRatio(String s, boolean max) {
		...
		if (valid)
			valid = (Integer.valueOf(nums[0]) == null || Integer.valueOf(nums[1]) == null);
		if (valid) {
			if (max)
				priceMax = false;
			else
